...
// I seperate capital letter classes from regulars
// Which is to say some classes are for inheritence and some for actual functionality
...
PixleeBot.prototype._onShare = function(message) {
    ...
    if (this._isValidMessage(message) && this._isFromCorrectChannel(message)) {
        ...
        var theDownloadFromSlack = request(message.file.url_private_download, {
            'auth': {
            ...
            s3.upload(s3Params,
                function(err, data) {
                    if (!err) {
                        ...
                        var pixleeUsername = message.username;
                        var pixleeEmail = _this._makeSlackUploadsEmail(pixleeUsername);
