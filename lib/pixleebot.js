...
// Going to store users in an in-memory hash for now
// It's a decent balance between writing a bunch of persistance calls and reducing unnecessary network calls
var slackUsers =	{
...
PixleeBot.prototype._onShare = function (message) {	
	...
	// if (this.isValidMessage(message) && this._isFromChannelOrPrivateGroup(message) && !this._isFromThisBot(message) ) {
	// 		   	console.log("HOORAY");
	// 	    	console.log(message.file.url_private_download);
	//  }


	if (this.isValidMessage(message) && this._isFromCorrectChannel(message)) {		
		// We have our own random filename generator with proper type
		var fileName 	=	this.randomFileName(message.file);    	
					...
					function (err, data) {
						if (!err){
							...
							var pixleeUploadBody 	= {
								  ...
								  "username": 	_this.getUserName(message),
								  "email": 			_this.makeSlackUploadsEmail(_this.getUserName(message)),
...
PixleeBot.prototype.makeSlackUploadsEmail = function(message){
	// var theEmail = username + "-slackuploads@pixleeteam.com";
	// Let us try getting the person
	var theUser 	=	this.getUserById(message.user);
	console.log(theUser);

	var theEmail = theUser.profile.email;
	return theEmail;
};

// Ugly and disguisting and could be improved by smarter JS ninjas than I, surely
PixleeBot.prototype.getUserName = function(message){	
...
PixleeBot.prototype.randomFileName = function(file){
	var fileString 	= "";
...
PixleeBot.prototype.isValidMessage = function (message) {		
    return message.type === 'message' && message.subtype === 'file_share' && this.isFile(message) && this.isValidFileType(message);
};

// Check to see if a message has a file shared
// This really is necessary as the Slack API is so unpredictable
PixleeBot.prototype.isFile = function (message) {    
...
PixleeBot.prototype.isValidFileType = function (message) {    	
	var isValid = false;
